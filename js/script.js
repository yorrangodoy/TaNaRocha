...
// -----------------------------------------------
// [HAPTIC] Vibração tátil — Fase 11
// -----------------------------------------------
const Haptic = {
  supported: 'vibrate' in navigator &&
             !window.matchMedia('(prefers-reduced-motion: reduce)').matches,
  light()     { if (this.supported) navigator.vibrate(10); },
  medium()    { if (this.supported) navigator.vibrate(25); },
  heavy()     { if (this.supported) navigator.vibrate(50); },
  success()   { if (this.supported) navigator.vibrate([20, 40, 30]); },
  error()     { if (this.supported) navigator.vibrate([60, 40, 60]); },
  selection() { if (this.supported) navigator.vibrate(8); },
};

// Vibração leve global em botões
document.addEventListener('pointerdown', (e) => {
  const btn = e.target.closest('button, .nav-item, .desktop-nav-item, .action-item');
  if (!btn) return;
  if (btn.classList.contains('nav-fab'))        Haptic.medium();
  else if (btn.classList.contains('btn-primary')) Haptic.medium();
  else if (btn.classList.contains('btn-danger'))  Haptic.heavy();
  else if (btn.classList.contains('nav-item') ||
           btn.classList.contains('desktop-nav-item')) Haptic.selection();
  else Haptic.light();
}, { passive: true });

// -----------------------------------------------
// [PWA INSTALL] Fase 12 — Gerenciador de instalação
// -----------------------------------------------
const PWAInstall = {
  deferredPrompt: null,
  platform: null,
  ...
  init() {
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      ...
      if (!this.wasDismissed()) this.showInstallUI();
    });
    ...
    window.addEventListener('appinstalled', () => {
      ...
      localStorage.setItem('tanarocha:pwa-installed', '1');
      showToast('📲 App instalado com sucesso!', 'success');
    });

    // iOS nunca dispara beforeinstallprompt — exibe botão permanente após delay
    if (this.platform === 'ios' && !this.wasDismissed()) {
      setTimeout(() => this.showInstallUI(), 2000);
    }
  },
  ...
  isInstalled() {
    return window.matchMedia('(display-mode: standalone)').matches
      ...
      || localStorage.getItem('tanarocha:pwa-installed') === '1';
  },

  wasDismissed() {
    const ts = localStorage.getItem('tanarocha:pwa-dismissed');
    if (!ts) return false;
  ...
  showInstallUI() {
    ...
    document.querySelectorAll('.btn-install-permanent').forEach(btn => {
  ...
  dismiss() {
    localStorage.setItem('tanarocha:pwa-dismissed', Date.now().toString());
    this.hideAllInstallUI();
    Haptic.light();
  },

  showIOSModal() {
    const modal = document.getElementById('install-ios-modal');
    if (modal) { modal.classList.add('open'); document.body.style.overflow = 'hidden'; }
  },

  showFallbackModal() {
    const modal = document.getElementById('install-fallback-modal');
    if (modal) { modal.classList.add('open'); document.body.style.overflow = 'hidden'; }
  },
};

function closeInstallModal(id) {
...
function showToast(msg, type = 'info', duration = 2800) {
  if (type === 'error') Haptic.error(); // [HAPTIC]
...
function getFrasePrincipal() {
  return 'Bora rachar!';
}

function popularPodioHome() {
  const container = document.getElementById('home-podium-row');
  if (!container) return;
  ...
  if (elegiveis.length === 0) {
    container.innerHTML = '<p class="text-white/40 text-xs text-center py-2">Faça rachadas para gerar ranking</p>';
    return;
  }

  const medals = ['🥇', '🥈', '🥉'];
  container.innerHTML = elegiveis.map((f, i) => `
    <div class="highlight-podium-spot">
      <div class="highlight-podium-medal">${medals[i]}</div>
      ${buildAvatarHTML(f.nome, 'w-11 h-11 text-sm mx-auto mb-1')}
      <div class="highlight-podium-name">${escapeHTML(f.nome.split(' ')[0])}</div>
    </div>
  `).join('');
}

function popularStoriesHome() {
  const container = document.getElementById('home-stories-row');
  if (!container) return;

  let html = `
    <div class="story-item story-add" id="btn-story-add-amigo" role="button" tabindex="0" aria-label="Adicionar amigo">
      <div class="story-add-circle" aria-hidden="true">+</div>
      <span class="story-name">Novo</span>
    </div>
  `;

  AppState.friends.slice(0, 8).forEach(f => {
    const score = calcularScore(f);
    const repClass = score >= 80 ? 'rep-confiavel' : score >= 50 ? 'rep-enrolado' : 'rep-caloteiro';
    const color = getAvatarColor(f.nome);
    const initials = getInitials(f.nome);
    html += `
      <div class="story-item">
        <div class="story-avatar ${repClass}" style="background-color:${color};" aria-hidden="true">${escapeHTML(initials)}</div>
        <span class="story-name">${escapeHTML(f.nome.split(' ')[0])}</span>
      </div>
    `;
  });

  container.innerHTML = html;
  document.getElementById('btn-story-add-amigo')?.addEventListener('click', openAddAmigoModal);
}
...
function popularFeedHome() {
  const container = document.getElementById('home-feed');
  if (!container) return;

  const items = AppState.history.slice(-5).reverse();

  if (items.length === 0) {
    container.innerHTML = '<p class="text-white/40 text-sm text-center py-4">Nenhuma atividade ainda</p>';
    return;
  }

  container.innerHTML = items.map(s => {
    const cat = getCategoryById(s.categoria || 'outro');
    return `
    <div class="feed-item">
      <div class="feed-item-icon icon-rachada" aria-hidden="true">${cat.icon}</div>
      <div class="feed-item-text">
        <div class="feed-item-title">${escapeHTML(s.eventName || 'Rachada')}</div>
        <div class="feed-item-time">${s.participants?.length || 0} participantes · ${formatarTempoRelativo(s.dateISO)}</div>
      </div>
    </div>
  `;
  }).join('');
...
function popularStatsHome() {
  const rachadas = AppState.history.length;
  const amigos   = AppState.friends.length;
  const badges   = computeBadges().length;

  const elR = document.getElementById('stat-rachadas');
  const elA = document.getElementById('stat-amigos');
  const elB = document.getElementById('stat-badges');

  if (elR) animateNumber(elR, 0, rachadas, 600);
  if (elA) animateNumber(elA, 0, amigos, 600);
  if (elB) animateNumber(elB, 0, badges, 600);
}

function checarEstadoVazio() {
  const temDados = AppState.friends.length > 0 || AppState.history.length > 0;
  document.getElementById('home-empty')?.classList.toggle('hidden', temDados);
  document.getElementById('home-content')?.classList.toggle('hidden', !temDados);
}

function inicializarHome() {
  const timeEl  = document.getElementById('greeting-time');
  const fraseEl = document.getElementById('greeting-frase');
  if (timeEl)  timeEl.textContent  = getSaudacao();
  if (fraseEl) fraseEl.textContent = getFrasePrincipal();

  checarEstadoVazio();

  if (AppState.friends.length > 0 || AppState.history.length > 0) {
    popularPodioHome();
    popularStoriesHome();
    popularFeedHome();
    popularStatsHome();
...
// [THEME] Cor da barra do sistema por tela — Fase 11
const THEME_COLORS = {
  'screen-home':        '#0D0D18',
  'screen-amigos':      '#0D0D18',
  'screen-historico':   '#0D0D18',
  'screen-podio':       '#1E1B4B',
  'screen-confronto':   '#2D1B4B',
  'screen-nova-sessao': '#6B3FF6',
  'screen-sobre':       '#1E1B4B',
};

function updateThemeColor(screenId) {
  const color = THEME_COLORS[screenId] || '#0D0D18';
...
function updateNavState(screenId) {
  const map = {
    'screen-home':        'home',
    'screen-amigos':      'amigos',
    'screen-nova-sessao': 'nova-rachada',
    'screen-podio':       'podio',
    'screen-confronto':   'confronto',
  ...
  document.querySelectorAll('.desktop-header [data-screen]').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.screen === key);
  });
}
...
// -----------------------------------------------
// FASE 11 — PULL-TO-REFRESH
// -----------------------------------------------

function setupPullToRefresh(screenId, onRefresh) {
  const screen = document.getElementById(screenId);
  if (!screen) return;

  // Injeta indicador no topo da tela alvo
  let indicator = screen.querySelector('.ptr-indicator');
  if (!indicator) {
    indicator = document.createElement('div');
    indicator.className = 'ptr-indicator';
    indicator.innerHTML = '<div class="ptr-spinner"></div><span class="ptr-text">Puxe para atualizar</span>';
  ...
  window.addEventListener('touchstart', (e) => {
    ...
    if (active?.id !== screenId || window.scrollY > 0 || isRefreshing) return;
  ...
  window.addEventListener('touchmove', (e) => {
    ...
    const distance = Math.min(Math.max(0, e.touches[0].clientY - startY) * 0.5, MAX_PULL);
    if (distance <= 0) return;

    screen.style.setProperty('--ptr-distance', `${distance}px`);
    screen.classList.add('ptr-wrapper', 'is-pulling');

    const spinner = indicator.querySelector('.ptr-spinner');
    if (spinner) spinner.style.transform = `rotate(${(distance / THRESHOLD) * 360}deg)`;

    if (distance >= THRESHOLD && !screen.classList.contains('ready-to-refresh')) {
      screen.classList.add('ready-to-refresh');
      Haptic.light();
    } else if (distance < THRESHOLD) {
      screen.classList.remove('ready-to-refresh');
    }
  }, { passive: true });
  ...
  window.addEventListener('touchend', () => {
    ...
    const distance = parseFloat(screen.style.getPropertyValue('--ptr-distance')) || 0;

    if (distance >= THRESHOLD) {
      isRefreshing = true;
      screen.classList.add('is-refreshing');
      screen.classList.remove('is-pulling', 'ready-to-refresh');
      Haptic.medium();

      Promise.resolve(onRefresh()).finally(() => {
        setTimeout(() => {
          screen.classList.remove('ptr-wrapper', 'is-refreshing');
          screen.style.removeProperty('--ptr-distance');
          isRefreshing = false;
        }, 400);
      });
    } else {
      screen.classList.remove('ptr-wrapper', 'is-pulling', 'ready-to-refresh');
      screen.style.removeProperty('--ptr-distance');
    }
  });
}
...
function initPullToRefresh() {
  setupPullToRefresh('screen-historico', () => new Promise(res => {
    renderHistorico();
    setTimeout(res, 600);
  }));
  setupPullToRefresh('screen-amigos', () => new Promise(res => {
    renderAmigos();
    setTimeout(res, 600);
  }));
}
...
function setupLongPress(element, onLongPress, duration = 500) {
  let timer = null;
  let startX = 0;
  let startY = 0;
  let triggered = false;
  ...
  // Evita click normal após long-press
...
function openActionSheet(title, actions) {
  ...
  actions.forEach(action => {
    ...
    btn.className = `action-item${action.danger ? ' danger' : ''}`;
    btn.innerHTML = `<span class="action-item-icon">${action.icon || '·'}</span><span>${escapeHTML(action.label)}</span>`;
    btn.addEventListener('click', () => {
      closeActionSheet();
    ...
    <div class="participant-card" id="card-${participant.id}" role="listitem">
      ...
      <div class="participant-card-body" id="body-${participant.id}">
        ...
        <div class="flex gap-2 mt-2">
          ...
          <input
            type="number"
            id="item-val-${participant.id}"
            class="item-val-input"
            value="0"
            min="0"
            step="any"
            inputmode="decimal"
            aria-label="Valor do item em reais"
            oninput="syncItemValueDraft('${participant.id}')"
          />
...
function changeItemValueDraft(participantId, delta) {
  ...
  if (el) el.value = itemValueDraft[participantId];
}

function syncItemValueDraft(participantId) {
  const el = document.getElementById(`item-val-${participantId}`);
  const val = el ? (parseFloat(el.value) || 0) : 0;
  itemValueDraft[participantId] = Math.max(0, val);
}
...
function addItem(participantId) {
  syncItemValueDraft(participantId);
...
function agruparPorDevedor(transfers) {
  const grupos = {};
  transfers.forEach(t => {
    if (!grupos[t.from]) {
      grupos[t.from] = { nome: t.from, transferencias: [], total: 0 };
    }
    grupos[t.from].transferencias.push(t);
    grupos[t.from].total = Math.round((grupos[t.from].total + t.amount) * 100) / 100;
  });
...
function renderTransfers(transfers) {
  ...
  const subtitulo = document.getElementById('pix-subtitulo');

  if (transfers.length === 0) {
    container.innerHTML = '<p class="pix-empty">Ninguém deve nada pra ninguém! 🎉</p>';
    if (subtitulo) subtitulo.textContent = '';
    return;
  }

  const grupos = agruparPorDevedor(transfers);
  const temMultiplos = grupos.some(g => g.transferencias.length > 1);

  if (subtitulo) {
    subtitulo.textContent = temMultiplos
      ? 'Alguns precisam fazer mais de um Pix'
      : 'Cada um faz só um Pix';
  }

  container.innerHTML = grupos.map(grupo => {
    const primeiroNome = escapeHTML(grupo.nome.split(' ')[0]);
    const isSingle = grupo.transferencias.length === 1;

    const linhas = grupo.transferencias.map(t => `
      <div class="pix-transfer-item">
        <span class="pix-transfer-arrow">→</span>
        <span class="pix-transfer-destino">${escapeHTML(t.to.split(' ')[0])}</span>
        <span class="pix-transfer-valor">${formatBRL(t.amount)}</span>
      </div>
    `).join('');

    return `
      <div class="pix-grupo${isSingle ? ' pix-grupo-single' : ''}" role="listitem"
           aria-label="${escapeHTML(grupo.nome)} deve ${formatBRL(grupo.total)} no total">
        <div class="pix-grupo-header">
          ${buildAvatarHTML(grupo.nome, 'sm')}
          <div class="pix-grupo-info">
            <span class="pix-grupo-nome">${primeiroNome}</span>
            <span class="pix-grupo-label">deve no total</span>
          </div>
          <span class="pix-grupo-valor">${formatBRL(grupo.total)}</span>
        </div>
        <div class="pix-grupo-transferencias">
          ${linhas}
        </div>
      </div>
    `;
...
function renderHistorico() {
  ...
  if (registrosFiltrados.length === 0) {
    ...
    list.innerHTML = registrosFiltrados.map((record, i) => {
      ...
      return `
        <div class="history-card list-item-stagger" role="article" data-record-id="${record.id}" aria-label="${escapeHTML(record.eventName)}, ${record.date}" style="animation-delay:${i * 55}ms">
    ...
    // [LONG_PRESS] Ações contextuais em sessões — Fase 11
    list.querySelectorAll('.history-card[data-record-id]').forEach(card => {
      setupLongPress(card, () => {
        const record = AppState.history.find(r => r.id === card.dataset.recordId);
        if (!record) return;
        openActionSheet(record.eventName || 'Sessão', [
          {
            icon: '📋',
            label: 'Copiar resumo',
            onClick: () => {
              const resumo = `${record.eventName} — ${record.date}\n${record.participants.length} pessoas · ${formatBRL(record.grandTotal)}`;
              navigator.clipboard?.writeText(resumo).then(() => showToast('📋 Resumo copiado!', 'success'));
              Haptic.medium();
            },
          },
          {
            ...
            danger: true,
            onClick: async () => {
              const ok = await confirmModal(`Excluir "${record.eventName}"?`, 'Essa ação não pode ser desfeita.');
              if (!ok) return;
              AppState.history = AppState.history.filter(r => r.id !== record.id);
              saveHistory();
              renderHistorico();
              showToast('🗑️ Sessão excluída', 'info');
            },
          },
        ]);
      });
...
function renderAmigos() {
  ...
  list.innerHTML = sorted.map((f, i) => `
    <div class="amigo-card list-item-stagger" role="listitem" data-friend-id="${f.id}" aria-label="${escapeHTML(f.nome)}, ${f.estatisticas.totalSessoes} sessões" style="animation-delay:${i * 55}ms">
      ${buildAvatarHTML(f.nome, 'lg')}
      <div class="flex-1 min-w-0">
        <p class="font-bold text-sm">${escapeHTML(f.nome)}</p>
        <p class="text-xs text-white/40 mt-0.5">
          ${f.estatisticas.totalSessoes !== 1 ? `${f.estatisticas.totalSessoes} sessões` : '1 sessão'}
  ...
  // [LONG_PRESS] Ações contextuais em amigos — Fase 11
  list.querySelectorAll('.amigo-card[data-friend-id]').forEach(card => {
    setupLongPress(card, () => {
      const friend = AppState.friends.find(f => f.id === card.dataset.friendId);
      if (!friend) return;
      openActionSheet(friend.nome, [
        {
          icon: '🗑️',
          label: 'Remover amigo',
          danger: true,
          onClick: async () => {
            const ok = await confirmModal(`Remover ${friend.nome}?`, 'O histórico não será afetado, mas o perfil será removido.');
            if (!ok) return;
            AppState.friends = AppState.friends.filter(f => f.id !== friend.id);
            saveFriends();
            renderAmigos();
...
// Índice = posição em DEMO_SESSIONS.participantes
// paySet: quais sessões (índice global) cada amigo pagou — determinístico para apresentação
// Scores resultantes: João 90%, Maria 80%, Lucas 83%, Pedro 60%, Carla 33%
const DEMO_FRIENDS = [
  { nome: 'João Silva',     paySet: new Set([0,1,2,3,4,5,7,8,10])     }, // 9/10 = 90%
  { nome: 'Maria Santos',   paySet: new Set([0,1,2,3,5,7,8,11])       }, // 8/10 = 80%
  { nome: 'Pedro Costa',    paySet: new Set([0,1,5,8,10,11])           }, // 6/10 = 60%
  { nome: 'Carla Souza',    paySet: new Set([0,5,11])                  }, // 3/9  = 33%
  { nome: 'Lucas Oliveira', paySet: new Set([1,3,5,10,11])             }, // 5/6  = 83%
];
...
async function activateDemoMode() {
  ...
  DEMO_SESSIONS.forEach((ds, sessionIndex) => {
    ...
    const sessionParticipants = ds.participantes.map((idx, i) => {
      ...
      const paid    = df.paySet.has(sessionIndex);
      return {
        id:        uid(),
        name:      friend.nome,
        profileId: friend.id,
...
// [Fase 12] initInstallBanner() substituída por PWAInstall.init() — ver seção PWA INSTALL no topo
...
function renderPodio() {
  ...
  const elegiveis = AppState.friends
    .filter(f => f?.nome && (f.estatisticas?.totalSessoes ?? 0) > 0)
    ...
    return `
      <div class="ranking-row list-item-stagger" role="listitem" aria-label="${pos}º lugar: ${escapeHTML(item.friend.nome)}, score ${item.score}%" style="animation-delay:${idx * 55}ms">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-white/40">${item.sessoes !== 1 ? `${item.sessoes} sessões` : '1 sessão'} · ${cat.icon} ${cat.nome}</p>
        </div>
...
function renderPodiumVisual(container, top3) {
  ...
  const spots = ordem.map((item, vi) => {
    if (!item) return '';
    const firstName = escapeHTML(item.friend.nome.split(' ')[0]);
    const pts       = `${item.score}pts`;
    const isFirst   = spotClass[vi] === 'first';
    return `
      <div class="podium-spot podium-${spotClass[vi]} appear"
           role="listitem"
           aria-label="${posLabels[vi]} lugar: ${escapeHTML(item.friend.nome)}, ${pts}">
        ${isFirst ? '<span class="podium-crown" aria-hidden="true">👑</span>' : ''}
        <div class="podium-info">
          ...
          <p class="podium-name">${firstName}</p>
          <span class="podium-pts">${pts}</span>
        </div>
        <div class="podium-degrau" aria-hidden="true">
          <span class="podium-degrau-label">${posLabels[vi]}</span>
        </div>
      </div>
    `;
  });
    ...
    <div class="guerreiro-grid">
      ${elegiveis.map(f => {
        ...
        return `
          ...
          >
            ${buildAvatarHTML(f.nome, 'sm')}
            <p class="guerreiro-nome">${escapeHTML(f.nome.split(' ')[0])}</p>
...
function renderConfrontoCards(p1, p2, cmp) {
  ...
  const cards = [
    ...
    },
    {
      ...
      data: cmp.sessoes,
      r1: () => cmp.sessoes.v1 !== 1 ? `${cmp.sessoes.v1} sessões` : '1 sessão',
      r2: () => cmp.sessoes.v2 !== 1 ? `${cmp.sessoes.v2} sessões` : '1 sessão',
      txt: {
  ...
  container.innerHTML = cards.map(card => {
    ...
    const vencedorNome = w1
      ? escapeHTML(p1.nome.split(' ')[0])
      : w2
        ? escapeHTML(p2.nome.split(' ')[0])
        : null;

    const cardClass = w1 ? 'winner-left' : w2 ? 'winner-right' : 'draw';

    return `
      <div class="confronto-card ${cardClass}" role="listitem">
        <div class="confronto-card-top">
          <span class="confronto-card-icon" aria-hidden="true">${card.icon}</span>
          <span class="confronto-card-label">${card.titulo}</span>
          ${isDraw
            ? '<span class="confronto-tag-empate">EMPATE</span>'
            : `<span class="confronto-tag-vence">${vencedorNome} vence</span>`
          }
        </div>
        <div class="confronto-card-valores">
          <span class="confronto-valor ${w1 ? 'winner' : ''}">${card.r1()}</span>
          <span class="confronto-vs-mini">vs</span>
          <span class="confronto-valor ${w2 ? 'winner' : ''}">${card.r2()}</span>
        </div>
        <p class="confronto-card-zoeira">${zoeira}</p>
      </div>
    `;
  }).join('');
}
...
function init() {
  ...
  document.getElementById('btn-home-demo')?.addEventListener('click', activateDemoMode);

  // Fase 11 — Action sheet
  document.getElementById('btn-close-action-sheet')?.addEventListener('click', closeActionSheet);
  document.getElementById('action-sheet')?.addEventListener('click', (e) => {
    if (e.target.id === 'action-sheet') closeActionSheet();
  });

  // Fase 12 — Modais de instalação PWA (backdrop fecha)
  ['install-ios-modal', 'install-fallback-modal'].forEach(id => {
    document.getElementById(id)?.addEventListener('click', (e) => {
      if (e.target.id === id) closeInstallModal(id);
    });
  });
  ...
  PWAInstall.init();    // [PWA Fase 12] banner + botões de instalação
...
window.syncItemValueDraft   = syncItemValueDraft;
